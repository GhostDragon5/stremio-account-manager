...
import {
  ...
  saveAddonLibrary,
} from '@/lib/addon-storage-backend'
import { normalizeTagName } from '@/lib/addon-validator'
import { resolveAccountAuthKey } from '@/lib/account-auth'
...
export const useAddonStore = create<AddonStore>((set, get) => ({
  ...
  updateLatestVersions: (versions) => {
    ...
    // Store in localStorage instead of localforage
    try {
      localStorage.setItem('stremio-manager:latest-versions', JSON.stringify(newVersions))
    } catch (error) {
      console.error('Failed to save latest versions:', error)
    }
  },
  ...
  createSavedAddon: async (name, installUrl, tags = [], existingManifest) => {
    ...
    try {
      ...
      const savedAddon: Omit<SavedAddon, 'id'> = {
        name: addonName,
        installUrl,
        manifest,
        tags: normalizedTags,
      ...
      // Create saved addon in backend
      const created = await backendApi.createSavedAddon(savedAddon)

      const library = { ...get().library, [created.id]: created }
      set({ library })
  ...
  updateSavedAddon: async (id, updates) => {
    ...
    try {
      ...
      // Update in backend
      await backendApi.updateSavedAddon(id, updatedSavedAddon)
  ...
  updateSavedAddonManifest: async (id) => {
    ...
    set({ latestVersions })
    try {
      localStorage.setItem('stremio-manager:latest-versions', JSON.stringify(latestVersions))
    } catch (error) {
      console.error('Failed to save latest versions:', error)
    }
  ...
  deleteSavedAddon: async (id) => {
    try {
      // Delete from backend
      await backendApi.deleteSavedAddon(id)
      ...
      const message = error instanceof Error ? error.message : 'Failed to delete saved addon'
      set({ error: message })
      throw error
    }
  },
  ...
  applySavedAddonToAccount: async (
    ...
    try {
      ...
      const authKey = await resolveAccountAuthKey(accountAuthKey)
  ...
  forceRemoveProtectedAddons: async (addonIds, accountIds) => {
    set({ loading: true, error: null })
    try {
      const result: BulkResult = {
        success: 0,
      ...
      for (const { id: accountId, authKey: accountAuthKey } of accountIds) {
        try {
          ...
          // Use the force remove function to disable protection and remove addons
          await forceRemoveProtectedAddons(authKey, addonIds)
      ...
      const message = error instanceof Error ? error.message : 'Failed to force remove protected addons'
      set({ error: message })
      throw error
    } finally {
      set({ loading: false })
  ...
  importLibrary: async (json, _merge) => {
    set({ loading: true, error: null })
    try {
      ...
      for (const savedAddon of savedAddons) {
        try {
          ...
          await backendApi.createSavedAddon(addonData)
        } catch (error) {
          console.error('Failed to import saved addon:', error)
        }
      }

      // Reload library from backend
      const library = await loadAddonLibrary()
      set({ library })
    } catch (error) {
  ...
  checkAllHealth: async () => {
    ...
    try {
      ...
      for (const addon of updatedAddons) {
        try {
          await backendApi.updateSavedAddon(addon.id, addon)
        } catch (error) {
          console.error('Failed to update addon health in backend:', error)
        }
      }
    } catch (error) {
