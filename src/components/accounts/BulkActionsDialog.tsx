...
export function BulkActionsDialog({ selectedAccounts, onClose }: BulkActionsDialogProps) {
  ...
  // Collect all unique addons across selected accounts, excluding protected ones for normal actions, but including them for force remove
  const allAddons = Array.from(
    new Map(
      selectedAccounts.flatMap((acc) =>
        acc.addons
          .filter((addon) => action === 'force-remove-addons' || !addon.flags?.protected)
    ...
    try {
      ...
      switch (action) {
        ...
        case 'add-by-tag':
          ...
          bulkResult = await bulkApplyTag(selectedTag, accountsData, strategy)
          break
        ...
        case 'force-remove-addons':
          if (selectedAddonIds.size === 0) {
            setError('Please select at least one addon to force remove')
            return
          }
          bulkResult = await forceRemoveProtectedAddons(Array.from(selectedAddonIds), accountsData)
          break
      }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <Select value={action} onValueChange={(v) => setAction(v as BulkAction)}>
          ...
          <SelectContent>
            ...
            <SelectItem value="force-remove-addons">Force Remove Protected Addon(s)</SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      {/* Force Remove Protected Addons */}
      {action === 'force-remove-addons' && (
        <div className="space-y-2">
          <Label>Select Protected Addons to Force Remove ({selectedAddonIds.size} selected)</Label>
          <p className="text-xs text-muted-foreground">
            This will temporarily disable protection and remove the selected addons. Use with caution.
          </p>
          <div className="border rounded-md max-h-64 overflow-y-auto">
            {allAddons.length === 0 ? (
              <p className="text-sm text-muted-foreground p-4 text-center">No protected addons available</p>
            ) : (
              <div className="divide-y">
                {allAddons.map((addon) => (
                  <label
