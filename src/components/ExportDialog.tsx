...
export function ExportDialog() {
  const isOpen = useUIStore((state: any) => state.isExportDialogOpen)
  const closeDialog = useUIStore((state: any) => state.closeExportDialog)
  const exportAccounts = useAccountStore((state: any) => state.exportAccounts)
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      a.href = url
      a.download = `stremio-accounts-${Date.now()}.json`
