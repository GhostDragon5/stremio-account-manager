...
export function AddonList({ accountId }: AddonListProps) {
  ...
  const updatesAvailable: AddonDescriptor[] = addons.filter((addon: AddonDescriptor) => {
  ...
  const handleUpdateAddon = useCallback(
    async (_accountId: string, addonId: string) => {
      ...
      const authKey = await resolveAccountAuthKey(account.authKey)
  ...
  const handleUpdateAll = useCallback(async () => {
    ...
    const addonsToUpdate = updatesAvailable.map((addon: AddonDescriptor) => addon.manifest.id)
    ...
    <div className="space-y-4">
      ...
      {addons.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {addons.map((addon: AddonDescriptor) => (
            <AddonCard
