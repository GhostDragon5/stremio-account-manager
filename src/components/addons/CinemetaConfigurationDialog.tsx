...
export function CinemetaConfigurationDialog({
  ...
  const handleApplyConfiguration = async () => {
    setApplying(true)
    try {
      // 1. Resolve auth key (handles decryption or raw fallback)
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      const originalManifest = await fetchOriginalCinemetaManifest(addon.transportUrl)

      // 2. Verify manifest name matches
      if (originalManifest.name !== 'Cinemeta') {
        throw new Error('Fetched manifest does not match Cinemeta')
      }

      // 3. Resolve auth key and update addon collection
      const authKey = await resolveAccountAuthKey(accountAuthKey)
      const currentAddons = await stremioClient.getAddonCollection(authKey)
      const cinemetaIndex = currentAddons.findIndex((a) => a.manifest.id === addon.manifest.id)
