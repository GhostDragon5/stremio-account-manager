...
import { Copy, ExternalLink, RefreshCw, Settings, ShieldAlert } from 'lucide-react'
...
export function AddonCard({
  ...
  const { library, createSavedAddon, forceRemoveProtectedAddons, loading: storeLoading } = useAddonStore()
  ...
  const [showForceRemoveDialog, setShowForceRemoveDialog] = useState(false)
  ...
  const handleForceRemove = async () => {
    ...
    try {
      await forceRemoveProtectedAddons([addon.manifest.id], [{ id: accountId, authKey: accountAuthKey }])
      toast({
        title: 'Addon Force Removed',
        description: `Successfully force removed ${addon.manifest.name}`,
      })
      setShowForceRemoveDialog(false)
      // Trigger a refresh by calling the remove handler
      onRemove(accountId, addon.manifest.id)
    } catch (error) {
      toast({
        title: 'Force Remove Failed',
        description: error instanceof Error ? error.message : 'Failed to force remove addon',
        variant: 'destructive',
      })
    } finally {
  ...
  return (
    <>
        ...
        <CardFooter className="flex flex-col gap-2">
          ...
          {isProtected && (
            <Button
              variant="destructive"
              size="sm"
              onClick={() => setShowForceRemoveDialog(true)}
              disabled={loading || forceRemoving}
              className="w-full"
            >
              <ShieldAlert className="h-4 w-4 mr-2" />
              {forceRemoving ? 'Force Removing...' : 'Force Remove'}
            </Button>
          )}
        </CardFooter>
      </Card>
      ...
      <ConfirmationDialog
        open={showForceRemoveDialog}
        onOpenChange={setShowForceRemoveDialog}
        title="Force Remove Protected Addon?"
        description={`This addon is protected and cannot be removed normally. Force remove will temporarily disable protection and remove "${addon.manifest.name}". This action cannot be undone.`}
